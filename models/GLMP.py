...
class GLMP(nn.Module):
    ...
    def generate(self,test):
        ...
        if args['dataset'] == 'kvr':
            genfile = 'data/Generate/KVR/{}gen.txt'.format(args["task"])
        else:
            genfile = 'data/Generate/dialog-bAbI-tasks/task{}gen.txt'.format(args["task"])
            ...
            for bi, row in enumerate(decoded_fine):
                ...
                for e in row:
                    if e == 'EOS':
                        ...
                        st_f += e + ' '
                for s in decoded_coarse[bi]:
                    if s == 'EOS':
                        break
                    else:
                        st_c += s + ' '
                # pred_sent = st.lstrip().rstrip()
                temp_gen = data_test['context_source'][bi]
                temp_gen += st_f + '\n' + st_c + '\n'
                conversation_gen.append(temp_gen)
        file_dir = os.path.split(genfile)[0]
        if not os.path.isdir(file_dir):
            os.makedirs(file_dir)
        if not os.path.exists(genfile):
            os.system(r"touch {}".format(genfile))
        with open(genfile, 'w', encoding='utf-8') as f:
            for conv in conversation_gen:
                f.write(conv)
                f.write('\n')
